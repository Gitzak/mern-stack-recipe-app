...
>>>>>>> kayzou
  ...
  async Login(req) {
    try {
      ...
      return sendResponse(
        CONSTANTS.USER_LOGIN_OK,
        CONSTANTS.SERVER_OK_HTTP_CODE,
        { token },
      );
    } catch (error) {
  ...
  async updateUser(req) {
    try {
      ...
      if (req.user.userId === id) {
        ...
        if (!user) {
          return sendResponse(
            ...
            CONSTANTS.SERVER_ERROR_HTTP_CODE,
          );
        } else {
      ...
      return sendResponse(
        ...
        CONSTANTS.SERVER_NOT_ALLOWED_HTTP_CODE,
      );
    } catch (error) {
      if (error.code === 11000) {
        ...
        return sendResponse(
          CONSTANTS.SERVER_ALREADY_EXISTS,
          CONSTANTS.SERVER_BAD_REQUEST_HTTP_CODE,
        );
      }
    }
  }
